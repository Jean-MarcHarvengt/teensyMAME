...
void rallyx_vh_screenrefresh(struct osd_bitmap *bitmap,int full_refresh)
{
	...
	for (offs = 0; offs < rallyx_radarram_size;offs++)
	{
		...
		{
			x = rallyx_radarcarx[offs] + ((~rallyx_radarcarcolor[offs] & 0x01) << 8);
			if (flipscreen) x -= 3;
		}
		else	/* Jungler */
			x = rallyx_radarcarx[offs] + ((~rallyx_radarcarcolor[offs] & 0x08) << 5) + 1;
		y = 238 - rallyx_radarcary[offs] - displacement;
...
if (flipscreen) x += 32;
		...
		drawgfx(bitmap,Machine->gfx[2],
				...
				0,0,
				x,y,
				flipscreen ? &spritevisibleareaflip : &spritevisiblearea,TRANSPARENCY_NONE,0);
	}
}
